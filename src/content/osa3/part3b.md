...
Let's return to our earlier attempt of using our new backend with the React frontend we made in [part 2](/osa2). Our earlier attempt got stuck at the following error message:
...
The GET request made by the frontend to the url <http://localhost:3001/notes> is not working for some reason. So what's the issue? The backend appears to work without any problems from the browser and postman but not our frontend application.
...
The problem is related to something called CORS or Cross-Orgin Resource Sharing. [Wikipedia](https://en.wikipedia.org/wiki/Cross-origin_resource_sharing) provides the following explanation:
...
In our context this means the following: the JavaScript code that our frontend application executes in the browser, is by default only allowed to communicate with servers that are in the same [origin](https://developer.mozilla.org/en-US/docs/Web/Security/Same-origin_policy). Because the backend server is running on port 3001 of localhost and the frontend application is being served from port 3000 of localhost, their origin is not the same.
...
CORS and [same origin policy](https://developer.mozilla.org/en-US/docs/Web/Security/Same-origin_policy) are not React or Node specific concepts. These concepts apply to all web applications.

<!-- Voimme sallia muista <i>origineista</i> tulevat pyynnöt käyttämällä Noden [cors](https://github.com/expressjs/cors)-middlewarea. -->
We can allow requests from other <i>origins</i> by using the [cors](https://github.com/expressjs/cors) middleware for express.

<!-- Asennetaan <i>cors</i> komennolla -->
Let's install <i>cors</i> with the command:
...
Let's import the middle ware and allow requests from all origins:
...
Now our frontend works again! However, our backend does not yet support toggling the importance of individual notes.

<!-- CORS:ista voi lukea tarkemmin esim. [Mozillan sivuilta](https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS). -->
You can read more about CORS from [Mozilla's webiste](https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS).
...
<i>If you have never used Heroku, I recommend that you read the [getting started](https://devcenter.heroku.com/start) guide on the Heroku website.</i>

<!-- Lisätään projektin juureen tiedosto <i>Procfile</i>, joka kertoo Herokulle, miten sovellus käynnistetään -->
Let's add a file named <i>Procfile</i> to the root of our application. Heroku will read the Procfile to find out how to start the application:
...
Let's change the very end of the <i>index.js</i> file where we defined the port of our server, so that is looks like this:
...
Now the _PORT_ variable will be assigned the value from the environment variable named _PORT_ if it is defined, and 3001 otherwise. Heroku configures the port of our application with the help of an environment variable.

<!-- Tehdään projektihakemistosta git-repositorio, lisätään <i>.gitignore</i> jolla seuraava sisältö -->
Let's turn our project directory into a git repository. Let's a <i>.gitignore</i> file with the following contents:
...
Let's create a Heroku application with the <i>heroku create</i> command.

<!-- Jos kaikki meni hyvin, sovellus toimii: -->
If everything worked like it should, the application works:

![](../images/3/25a.png)

<!-- Jos ei, vikaa voi selvittää herokun lokeja lukemalla, eli komennolla <i>heroku logs</i>. -->
If the application doesn't work, you can try to figure out what's wrong from the logs with the <i>heroku log</i> command.
...
**NB** it's still a smart idea to follow the logs of the Heroku application. The easiest way of accomplishing this is with the <i>heroku logs</i> command, that will display output as things happen on the server.
...
Our frontend application also works with the backend server running in Heroku. You can verify this by changing the address of the backend defined in the frontend from <i>http://localhost:3001</i> to the address of your Heroku backend.
...
Naturally, the next question to ask is how do we deploy our frontend to the internet? There are many options, and next we will take a look at one of them.
...
So far we have been running our React application in <i>development mode</i>, where the application is configured to give highly detailed error messages and to hot reload the application in the browser when changes are made in the source code.
...
When the application is deployed to production, we have to make a [production build](https://reactjs.org/docs/optimizing-performance.html#use-the-production-build) that is optimized for the production environment.
...
React applications that are created with <i>create-react-app</i> come with an [npm run build](https://github.com/facebookincubator/create-react-app#npm-run-build-or-yarn-build) command that can be used for creating a production optimized build of the application.

<!-- Suoritetaan nyt komento <i>frontendin projektin juuressa</i>. -->
Let's run the command in the <i>root directory of the frontend project</i>.
...
Running the command creates a <i>build</i> directory that contains the only HTML file <i>index.html</i> of our application. Additionally, <i>build</i> contains the <i>static</i> directory, where the application's [minified](<https://en.wikipedia.org/wiki/Minification_(programming)>) JavaScript code is generated. Even though the application's code is written in many files, once it is built, all of the JavaScript code is generated into a single file that also contains the code of all of the required dependencies.

<!-- Minifioitu koodi ei ole miellyttävää luettavaa. Koodin alku näyttää seuraavalta: -->
Minified code is not very pretty to look at. The beginning of the code looks like this: 
...
One possibility for deploying the frontend code to production is to copy the production code in the generated <i>build</i> directory, into the root of the backend repository. Then we define the backend to display as its <i>main page</i> the <i>build/index.html</i> file in the generated production code.
...
Let's start by copying the frontend production code to the root of the backend project. On my machine, copying is accomplished by calling the following command from the frontend directory:
...
The contents of the backend directory should look something like this:
...
In order to be able to display <i>static content</i> like our <i>index.html</i> page and its associated JavaScript files etc., we need to use the built-in [static](http://expressjs.com/en/starter/static-files.html) middleware from express.

<!-- Kun lisäämme muiden middlewarejen määrittelyn yhteyteen seuraavan -->
We can take the middleware into use by adding the following into our other middleware definitions:
...
So what does the middleware actually do? For every HTTP GET request express will check if there is a file in the <i>build</i> directory that matches the request URL. If there if a matching file, then express returns that file.
